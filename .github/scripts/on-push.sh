...
function build(){
    ...
    local BUILD_SKETCHES="${SCRIPTS_DIR}/sketch_utils.sh sbuild"
...
if [ "$BUILD_PIO" -eq 0 ]; then
    ...
    SKETCHES_ESP32="\
      ...
      $ARDUINO_ESP32_PATH/libraries/BluetoothSerial/examples/SerialToSerialBT/SerialToSerialBT.ino\
    "
    ...
    build "esp32" $FQBN_ESP32 $CHUNK_IDEX $CHUNKS_CNT $SKETCHES_ESP32
    build "esp32s2" $FQBN_ESP32S2 $CHUNK_IDEX $CHUNKS_CNT $SKETCHES_ESP32XX
    build "esp32c3" $FQBN_ESP32C3 $CHUNK_IDEX $CHUNKS_CNT $SKETCHES_ESP32XX
else
